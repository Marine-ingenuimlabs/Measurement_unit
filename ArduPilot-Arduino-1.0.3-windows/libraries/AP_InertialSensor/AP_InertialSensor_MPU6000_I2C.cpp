 ...
 */
//const float AP_InertialSensor_MPU6000_I2C::_gyro_scale = (0.0174532 / 16.4); //2000dps
const float AP_InertialSensor_MPU6000_I2C::_gyro_scale = (0.0174532 / 65.5);//500dps
/* 
   ...
   variants however
 */
//const float AP_InertialSensor_MPU6000_I2C::_accel_scale = 9.81 / 4096.0;//8G
const float AP_InertialSensor_MPU6000_I2C::_accel_scale = 9.81 / 16384.0;//2G
...
void AP_InertialSensor_MPU6000_I2C::hardware_init()
{
    ...
    delay(1);
		...
		//	return;
			if (I2c.write(mpu_addr, MPUREG_GYRO_CONFIG, BITS_FS_500DPS) != 0) { // Gyro scale 500º/s
			return;
		} 	
    delay(1);
//		if (I2c.write(MPU6000_ADDR, MPUREG_ACCEL_CONFIG, 0x08) != 0) { // Accel scale 4g (4096LSB/g)
		...
		if ( (_product_id == MPU6000_REV_A4) || (_product_id == MPU6000ES_REV_C4) || (_product_id == MPU6000ES_REV_C5) ||
			...
			//	return;
				if (I2c.write(mpu_addr, MPUREG_ACCEL_CONFIG, 0) != 0) { // Accel scale 2g (16384LSB/g)
				return;
			} 	
		} else {
			// Accel scale 8g (4096 LSB/g)
